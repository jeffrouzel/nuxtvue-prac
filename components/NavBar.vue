<template>
  <div class="flex w-screen h-[80px] text-gray-500">
  
    <div class="w-[20%]">
      <div class="flex pl-5 items-center h-[100%] gap-4">
        <img src="assets/logo.png" alt="seavxr" class="w-[50px] h-[50px]" />
        <div class="flex text-2xl font-semibold">
          <h1 class="text-blue-600">Seav</h1>
          <h1>XR</h1>
        </div>
      </div>
    </div>

    <div class="flex gap-12 w-[80%] items-center pl-10 shadow-bottom">
      <NuxtLink to="/home" class="hover:text-blue-500">Home</NuxtLink>
      <NuxtLink to="/courses" class="hover:text-blue-500">Courses</NuxtLink>
      <NuxtLink to="/blog" class="hover:text-blue-500">Blog</NuxtLink>
      <NuxtLink to="/contact" class="hover:text-blue-500">Contact</NuxtLink>
    </div>

  </div>
</template>